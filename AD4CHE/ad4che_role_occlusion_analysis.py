...
class Config:
    ...
    MAIN_LANE_IDS: Set[int] = field(default_factory=lambda: set(range(1, 12)) | {101, 102, 103, 104, 105})
...
class AD4CHERoleLoader:
    ...
    def is_lane_allowed(self, lane_id: Optional[int]) -> bool:
        """Check if a lane is within the main-lane groups we care about."""
        if lane_id is None:
            return False
        try:
            return int(lane_id) in self.config.MAIN_LANE_IDS
        except (TypeError, ValueError):
            return False
    ...
    def filter_main_lane_agents(self, agents: List[AgentState]) -> List[AgentState]:
        """Keep only agents whose lane is in the configured main-lane set."""
        return [a for a in agents if self.is_lane_allowed(a.lane_id)]
...
def analyze_recording(data_dir: str, recording_id: int,
    ...
    main_lane_agents = loader.filter_main_lane_agents(agents)
    agents_for_view = main_lane_agents
    if ego_id is not None:
        agents_for_view = loader.select_surrounding_agents(main_lane_agents, ego_id, limit=max_surrounding)
    ...
    if include_occlusion:
        agent_dicts = [
            {'id': a.id, 'x': a.x, 'y': a.y, 'heading': a.heading,
             ...
             'driving_direction': a.driving_direction, 'lane_id': a.lane_id}
            for a in agents_for_view
        ]
        all_occlusions = loader.occlusion_detector.find_all_occlusions(agent_dicts, frame)
        logger.info(f"Found {len(all_occlusions)} occlusions (same direction, main lanes only)")
    else:
    ...
    viz.plot_traffic_snapshot(ax1, agents_for_view, all_occlusions, ego_id, use_full_extent=True)
    fig1.tight_layout()
    ...
    if ego_id is not None:
        ...
        viz.plot_ego_occlusion_view(ax2, agents_for_view, ego_occlusions, ego_id, use_full_extent=True)
        fig2.tight_layout()
    ...
    viz.plot_role_distribution(ax3, agents_for_view)
    fig3.tight_layout()
    ...
    viz.plot_summary(ax5, agents_for_view, all_occlusions, ego_id, frame)
    fig5.tight_layout()
    ...
    viz.plot_traffic_snapshot(ax_c1, agents_for_view, all_occlusions, ego_id, use_full_extent=True)
    viz.plot_role_distribution(ax_c2, agents_for_view)
    if ego_id is not None:
        viz.plot_ego_occlusion_view(ax_c3, agents_for_view, ego_occlusions, ego_id, use_full_extent=True)
    else:
    ...
    if include_occlusion:
        ...
        if log_all_frames:
            ...
            for f in all_frames[::5]:  # Sample every 5 frames
                ...
                allowed_f_agents = [a for a in f_agent_dicts if loader.is_lane_allowed(a.get('lane_id'))]
                f_occlusions = loader.occlusion_detector.find_all_occlusions(allowed_f_agents, f)
    ...
    metadata = {
        ...
        'total_agents': len(agents_for_view),
...
def _create_animation(loader: AD4CHERoleLoader, viz: RoleOcclusionVisualizer,
    ...
    # Use full recording range (subsampled by step)
    frames = frames_available[::max(step, 1)]
    ...
    for frame_id in frames:
        ...
        agents = loader.filter_main_lane_agents(agents)
        if ego_id is not None:
            agents = loader.select_surrounding_agents(agents, ego_id, limit=6)
        agent_dicts = [
            {'id': a.id, 'x': a.x, 'y': a.y, 'heading': a.heading,
             'vx': a.vx, 'vy': a.vy,
             'width': a.width, 'length': a.length, 'class': a.vehicle_class,
        ...
        occlusions = []
        if include_occlusion:
            allowed_agents = [a for a in agent_dicts if loader.is_lane_allowed(a.get('lane_id'))]
            occlusions = loader.occlusion_detector.find_all_occlusions(allowed_agents, frame_id)
